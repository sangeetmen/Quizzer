...
function getUploadedWritList() {
	...
	$
			.ajax({
				type : "POST",
				url : 'ewrit/dispatch/getDispatchList',
				dataType : "json",
				data : ({
				...
				success : function(data) {
					...
					for (var j = 0; j < response.length; j++) {
						...
						displayhtml += "<tr><td><i class='fas fa-check upldbtn' title='Approve eWrit'  data-toggle='modal' data-target='#modal-default' onclick=confirmappewrit('"
								+ response[j].wid.wid
								+ "','"
								+ response[j].trackid + "','F');>&nbsp;</i>";

						displayhtml += "<i class='fas fa-times dclinbtn' title='Decline eWrit' data-toggle='modal' data-target='#modal-default' onclick=confirmappewrit('"
								+ response[j].wid.wid
								+ "','"
...
function approvewrit() {
	...
	$.ajax({
		type : 'POST',
		url : 'ewrit/dispatch/approvewrithc',
		data : ({
			trackid : hdntrackid,
			wid : hdnwid,
			action : hdnaction
